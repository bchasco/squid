...
for(subDat in mySurveys){
  ...
  strata.limits <- data.frame(
    'STRATA' = c("OR","WA"),
    'north_border' = c(46.0, 49.0),
    'south_border' = c(42.0, 46.0)
  )
...
JSOES_fit$parameter_estimates$diagnostics[grep("lam",names(JSOES_fit$parameter_estimates$par)),]

for(i in c(1,2,3,4,5,6,7)){
  plot_results(fit,plot_set = i)
}


for(i in c(1,2,3,4,5,6,7)){
  plot_results(fit,plot_set = i, years_to_plot = c(1,7,14,21)
  )
}

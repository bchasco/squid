...
lat <- data.frame(lat=as.numeric(round(survey$Lat,1)))
lat_range <- round(seq(min(as.integer(floor(lat$lat)*10)),
                       max(as.integer(ceiling(lat$lat)*10)),1),
                   1)/10
...
try(dyn.unload("squid_v1_delta_glm_AR1yr_AR1Dspace"))
compile("squid_v1_delta_glm_AR1yr_AR1Dspace.cpp")
dyn.load("squid_v1_delta_glm_AR1yr_AR1Dspace")
rm(out)
...
Parameters = list(beta_c = rep(0,ncol(Data$X_xp)),
                  ...
                  eps_p_yl = array(0,c(length(lati_lu$lat_i),length(unique(Data$t_i)))),
                  eps_c_yl = array(0,c(length(lati_lu$lat_i),length(unique(Data$t_i)))),
                  fc_y_sd = 0,
                  fp_y_sd = 0,
                  fc_y_rho = 0,
                  fp_y_rho = 0,
...
Obj = MakeADFun(data=Data,
                ...
                map = myMap,
                hessian=FALSE,
                DLL="squid_v1_delta_glm_AR1yr_AR1Dspace")

...
# col_seq <- seq(floor(min(rep$pred_p_ef)),ceiling(max(rep$pred_p_ef)),abs(floor(min(rep$pred_p_ef))-ceiling(max(rep$pred_p_ef)))/50)
col_seq <- plogis(seq(floor(min(rep$beta_p[1]+rep$pred_p_ef)),ceiling(max(rep$beta_p[1]+rep$pred_p_ef)),
               abs(ceiling(max(rep$beta_p[1]+rep$pred_p_ef))-floor(min(rep$beta_p[1]+rep$pred_p_ef)))/50))
par(mfrow=c(5,5))
...
for(i in 1:22){
  ...
  x <- plogis(rep$beta_p[1]+rep$pred_p_ef[,,i])

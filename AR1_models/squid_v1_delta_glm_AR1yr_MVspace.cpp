...
Type objective_function<Type>::operator() ()
{
  ...
  Type c_lat_rho = atan(fc_lat_rho)*2./3.154;//1/(1+exp(-1.0*fc_lat_rho));
  Type p_lat_rho = atan(fp_lat_rho)*2./3.154;//1/(1+exp(-1.0*fp_lat_rho));
  ...
  for(int i = 0; i < n; i++){
    for(int j = 0; j < n; j++){
      Sigma_c(i,j) = pow(c_lat_rho,abs(lat_dist(i)-lat_dist(j)));
      Sigma_p(i,j) = pow(p_lat_rho,abs(lat_dist(i)-lat_dist(j)));
    }
  }
  if(use_c_lat){
    jnll_comp(0) += MVNORM(Sigma_c)(eps_c_lat);
  }
  if(use_p_lat){
    jnll_comp(0) += MVNORM(Sigma_p)(eps_p_lat);
  }

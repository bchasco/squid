...
for(subDat in mySurveys){
  ...
  FieldConfig = c("Omega1" = 0 #spatial detection
                  ,"Epsilon1" = 1 #spatiotemporal detection
                  ,"Omega2" = 0 #spatial positive
  ...
  RhoConfig= c("Beta1" = 0#Temporal corr. encounter covariate intercepts, 0 SWFSC, , 0 or 1 All
               ,"Beta2" = 0 #Temporal corr. for positive catch covariate intercepts, 0 SWFSC, , 0 or 1 All
...
SWFSC_fit$parameter_estimates$diagnostics[grep("lam",names(SWFSC_fit$parameter_estimates$par)),]

for(i in c(1,2,3,4,5,6,7)){
  plot_results(SWFSC_fit,plot_set = i)
}


plot_results(SWFSC_fit,plot_set = i, years_to_plot = c(1,7,14,21)
)

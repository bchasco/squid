...
Type objective_function<Type>::operator() ()
{
  ...
  Type p_yl_rho_lat = 1/(1+exp(-1.0*fp_yl_rho_lat));//f(fp_yl_rho_lat);//atan(fp_yl_rho_lat)*2./3.154;//
  ...
  if(use_c_yl){
    jnll_comp(0) += SEPARABLE(AR1(c_yl_rho_y),AR1(c_yl_rho_lat))(eps_c_yl);
  }
  
  if(use_p_yl){
    jnll_comp(0) += SEPARABLE(AR1(p_yl_rho_y),AR1(p_yl_rho_lat))(eps_p_yl);
  }

...
grid_size_km <- 175
n_x <- 100 #number of knots.  This is really important. To few and the model won't converge, too many and it will take forever.
...
ii <- 1
for(k_sub in 1:1){
  for(q in 1:1){#0:1include environmental covariates
    for(k in c(0)){#c(0,1,2,4) temporal correlation
